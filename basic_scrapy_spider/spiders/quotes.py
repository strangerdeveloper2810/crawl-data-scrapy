...
class AmazonSearchProductSpider(scrapy.Spider):
    ...
    def discover_product_urls(self, response):
        ...
        search_products = response.css(
            "div.s-result-item.s-asin[data-component-type=s-search-result]")
    ...
    def parse_product_data(self, response):
        ...
        # feature_bullets = [bullet.strip() for bullet in response.css(
        #     "#feature-bullets li ::text").getall()]
        price = response.css(
            '.a-price span[aria-hidden="true"] ::text').get("")
        if not price:
